    ...
    for i in range(3) :
        for j in range(3):
            if n_board[i][j]==-1:
                cpt1=cpt1+1
    return cpt1
def is_new(n_board,copie_board):
    if np.array_equal(n_board, copie_board):
        print("is_new false1")
        return False
    else:
        if len(comp_history)==3:
            if cmbdemoins1(n_board)==3:
                print("is_new true")
                return True
            else:
                print("is_new false2")
...
def data2move(data,copie_board):
    if bad_value(data) == False:
        ...
        if is_new(n_board,copie_board) == True and is_correct(n_board) == True and un_coup(n_board)==True:
...
def human_turn(c_choice, h_choice):
    ...
    while h_move == -1:
        try:
            ...
            print("données des joueurs : ")
            print(data)
            
            h_move = data2move(data,copie_board)
            if h_move == -1:
                print('\nBad move 1')
                render(board, c_choice, h_choice)
                send("-1")
                ...
                human_history.append((coord[0], coord[1]))
