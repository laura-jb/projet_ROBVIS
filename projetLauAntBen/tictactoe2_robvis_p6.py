...
def minimax(state, depth, player,Movetochange):
    """
    ...
    statetochange = state.copy()
    if Movetochange!=0:
        COUPLE=moves2[str(Movetochange)]
        (x,y)=COUPLE
        statetochange[x][y]=0
    ## #### #### ######## ######
    if player == COMP:
    ...
    if depth == 0 or game_over(statetochange):
        score = evaluate(statetochange)
        return [-1, -1, score]

    for cell in empty_cells(state):
        x, y = cell[0], cell[1]
        statetochange[x][y] = player
...
def ai_turn(c_choice, h_choice,Movetochange):
    moves = {
        "00" :1 , "01" :2 , "02" :3 ,
        "10" :4 , "11" :5 , "12" :6 ,
        "20" :7 , "21" :8 , "22" :9 ,
    ...
    if depth == 9:
        ...
        move = minimax(board, depth, COMP,Movetochange)
        x, y = move[0], move[1]
...
try:
    while 1:
        ...
        while len(empty_cells(board)) > 0 and not game_over(board):
            if first == 'N':
                ai_move = ai_turn(c_choice, h_choice,Listecoups[2])
                Listecoups[0]=ai_move
                NbTour=NbTour+1
                first = ''
            ...
            send(str(ai_move))
            Listecoups[0],Listecoups[1],Listecoups[2]=ai_move,Listecoups[0],Listecoups[1]
